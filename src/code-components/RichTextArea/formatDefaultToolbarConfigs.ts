...
export function formatDefaultToolbarConfigs(toolbar: Toolbar): ToolbarConfigs {
    ...
    if (toolbar.textStyle) {
      formattedToolbar.push(toolbar.textStyle.map(style => 
    ...
    if (toolbar.heading) {
      const headerMapping: Record<string, number | false> = {
        ...
        "Heading 4": 4, "Heading 5": 5, "Heading 6": 6, "Body": false
      };
      const filteredHeaders = toolbar.heading.map(h => headerMapping[h]).filter(h => h !== undefined);
